...
impl FusedChainFunctional {
    pub fn new_dimer(sigma1: f64, sigma2: f64, distance: f64) -> DFT<Self> {
        ...
        contributions.push(Box::new(FMTFunctional::new(&parameters)));
    ...
    pub fn new_monomer(sigma: f64) -> DFT<Self> {
...
impl<N: DualNum<f64>> FunctionalContributionDual<N> for FMTFunctional {
    fn weight_functions(&self, _: N) -> WeightFunctionInfo<N> {
        ...
        WeightFunctionInfo::new(self.parameters.n_segments.clone(), false)
            ...
            .add(
                WeightFunction {
                    prefactor: Zip::from(&self.parameters.m_a)
                        .and(&r)
            ...
            .add(
                WeightFunction {
                    prefactor: self.parameters.m_a.mapv(N::from),
            ...
            .add(
                WeightFunction {
                    prefactor: self.parameters.m_v.mapv(N::from),
            ...
            .add(
                WeightFunction {
                    prefactor: Zip::from(&self.parameters.m_v)
                        .and(&r)
                        .map_collect(|&m, &r| r.recip() * m / (4.0 * PI)),
                    kernel_radius: r.clone(),
    ...
    for FusedSegmentChainFunctional
{
    fn weight_functions(&self, _: N) -> WeightFunctionInfo<N> {
        ...
        WeightFunctionInfo::new(self.parameters.n_segments.clone(), true)
            .add(
                WeightFunction {
                    prefactor: self.parameters.m_a.mapv(|m| m.into()) / (&d * 8.0),
            ...
            .add(
                WeightFunction {
                    prefactor: self.parameters.m_v.mapv(|m| (m / 8.0).into()),
                    kernel_radius: d,
                    shape: WeightFunctionShape::Theta,
                },
                true,
...
mod tests {
    ...
    fn test_fused_chain_functional() -> EosResult<()> {
        let func = Rc::new(FusedChainFunctional::new_dimer(1.0, 1.0, 1.0));
        let bulk = StateBuilder::new(&func)
            .temperature(100.0 * KELVIN)
            .density(0.05 * MOL / METER.powi(3))
            .build()?;
        let profile = AdsorptionProfile::new_slit_pore(
            &bulk,
            1024,
            100.0 * ANGSTROM,
